...
```javascript
const roles = {
  Admin: '超级管理员',      // 拥有所有权限，系统最高权限
  Manager: '管理员',        // 拥有业务管理权限，包含原财务、人事、运营、票务管理员
  Employee: '普通员工'      // 拥有基础操作权限，包含原客服人员等
}
```
...
| 角色 | 权限范围 | 说明 |
|------|----------|------|
| **Admin** | 全部权限 | 系统超级管理员，拥有所有模块的完整权限，包括用户管理、角色权限配置等 |
| **Manager** | 业务管理权限 | 可管理票务、财务、人力资源、设施等业务模块，但无法修改系统权限配置 |
| **Employee** | 基础操作权限 | 可进行游客管理、基础查询等日常操作，权限相对受限 |

#### 角色映射关系

为保持向后兼容，原有角色按以下规则映射到新角色：
