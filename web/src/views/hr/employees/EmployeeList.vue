      ...
      <el-table :data="tableData" v-loading="loading" stripe border style="width: 100%">
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="{ row }">
            ...
            <el-button type="danger" size="small" @click="handleDelete(row)" :disabled="row.employmentStatus !== 'Active'">
              离职
            </el-button>
          </template>
        </el-table-column>
...
const loadData = async () => {
  ...
  try {
    ...
    if (searchForm.name || searchForm.department || searchForm.position) {
      const parts = [searchForm.name, searchForm.department, searchForm.position]
