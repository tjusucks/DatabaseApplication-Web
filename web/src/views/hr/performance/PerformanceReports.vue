...
const loadData = async () => {
  ...
  try {
    ...
    poorReviews.value = [...reviews]
      .filter(
        (review) => review.score < 70 || review.evaluationLevel === "不合格",
      )
      .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
      .slice(0, 10);
    ...
    statistics.value.excellentReviews = reviews.filter(
      (review) => review.score >= 90,
    ).length;
