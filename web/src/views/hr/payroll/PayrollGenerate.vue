  ...
  <div class="payroll-records">
    ...
    <el-dialog v-model="detailDialogVisible" title="工资单详情" width="600px">
      <el-descriptions :column="1" border>
        <el-descriptions-item label="员工姓名">{{ currentPayroll.employeeName }}</el-descriptions-item>
        <el-descriptions-item label="员工编号">{{ currentPayroll.staffNumber }}</el-descriptions-item>
        <el-descriptions-item label="部门">{{ currentPayroll.departmentName }}</el-descriptions-item>
        <el-descriptions-item label="职位">{{ currentPayroll.position }}</el-descriptions-item>
        <el-descriptions-item label="发放日期">{{ formatDate(currentPayroll.payDate) }}</el-descriptions-item>
        <el-descriptions-item label="基本工资">¥{{ currentPayroll.salary?.toFixed(2) }}</el-descriptions-item>
      </el-descriptions>
      <template #footer>
    ...
    <!-- 生成工资单对话框 -->
    <el-dialog v-model="generateDialogVisible" title="生成工资单" width="600px" @close="resetGenerateForm">
      <el-form 
        ref="generateFormRef" 
        :model="generateForm" 
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="submitGenerateForm" :loading="generateLoading">确定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
