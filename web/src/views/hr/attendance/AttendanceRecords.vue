<template>
  <div class="attendance-records">
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="scope">
            <el-button
              type="primary"
              size="small"
              @click="selectEmployee(scope.row)"
            >
              查看考勤
            </el-button>
          </template>
    ...
    <div v-else>
      <el-page-header
        @back="backToEmployeeList"
        :content="`${selectedEmployee.user.displayName} 的考勤记录`"
        class="mb-20"
      />
      ...
      <el-card class="search-card mb-20">
        <el-form
          ...
          label-width="100px"
          @submit.prevent="searchAttendance"
          inline
        >
    ...
    >
      ...
      >
        <el-form-item label="员工ID" prop="employeeId">
          <el-input
            v-model.number="attendanceForm.employeeId"
        ...
        <el-form-item label="考勤状态" prop="attendanceStatus">
          <el-select
            v-model="attendanceForm.attendanceStatus"
            placeholder="请选择考勤状态"
            style="width: 100%"
          >
        ...
        <el-form-item label="请假类型" prop="leaveType">
          <el-select
            v-model="attendanceForm.leaveType"
            placeholder="请选择请假类型"
            style="width: 100%"
          >
...
const handleCheckIn = async () => {
  ...
  try {
    ...
    ElMessage.error(
      "补签失败: " + (error.response?.data?.message || error.message),
    );
  } finally {
    checkInLoading.value = false;
  }
