    ...
    <el-card shadow="never" class="form-card">
      <el-form :model="saleForm" :rules="rules" ref="saleFormRef" label-width="120px">
        <el-form-item label="门票类型" prop="ticketTypeId">
          <el-select v-model="saleForm.ticketTypeId" placeholder="请选择门票类型" style="width: 100%" filterable
            v-loading="loadingTicketTypes">
            <!-- [最终修正] 确保这里的 item.xxx 与后端返回的 JSON 字段名完全一致 -->
            <el-option v-for="item in ticketTypes" :key="item.ticketTypeId"
              :label="`${item.typeName} (¥${item.basePrice})`" :value="item.ticketTypeId" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item class="form-footer">
          ...
          <el-button type="primary" @click="handleSubmit" :loading="isSubmitting">确认销售</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </PageTemplate>
