        ...
        <el-table-column label="操作" width="200">
          <template #default="{ row }">
            ...
            <el-button
              size="small"
              type="success"
              @click.stop="executeSchedule(row)"
              v-if="!row.isCompleted"
              >执行</el-button
            >
            <el-button size="small" type="danger" @click.stop="deleteSchedule(row.maintenanceId)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="isExecuteDialogVisible" title="执行维护计划" width="500px">
      <el-form :model="executeForm" ref="executeFormRef" label-width="120px">
        ...
        <el-form-item label="更换部件">
          <el-input
            v-model="executeForm.partsReplaced"
            type="textarea"
        ...
        <el-form-item label="维护详情">
          <el-input
            v-model="executeForm.maintenanceDetails"
            type="textarea"
            placeholder="请输入实际维护详情"
          />
        </el-form-item>
      </el-form>
